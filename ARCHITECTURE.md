...
Layer 3a — Investigation Agent
  Model: claude-haiku-4-5 (langchain-anthropic)
...
Layer 3b — Critical Agent
  ...
  RAG: BDDK/FATF PDFs → Qdrant, dense retrieval, citation mandatory
...
POST /transactions
  ...
  → Triage Router
    ...
    → investigate: run_fraud_investigation(graph.py) called
        ...
        → response returned
    → escalate: Critical Agent (RAG + network analysis)
        → Synthesizer: FraudDecision produced
        → SAR Generator: STR report produced (escalate only)
...
LLMs:           claude-haiku-4-5 (all agent roles)
Observability:  LangSmith, MLflow, structlog
DevOps:         Docker Compose, GitHub Actions, uv, ruff
...
4. claude-haiku-4-5 for all agent roles (investigation, critical, synthesis, SAR) — single model, cost optimized via tier routing
