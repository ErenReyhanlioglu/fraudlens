#!/usr/bin/env python
"""FraudLens Investigation Agent health probe.

Usage:
    uv run scripts/investigator_agent_healthcheck.py [--port PORT] [--host HOST]
                                                     [--no-langsmith]
                                                     [--reliability-samples N]
                                                     [--seed S]

Sections:
  INFRASTRUCTURE        — TCP reachability for PostgreSQL and Redis
  TRIAGE ROUTING        — 1 POST per investigate bucket, validates expected triage_action
  AGENT INVESTIGATION   — detailed tool trace + verdict for investigate buckets
  STRUCTURED OUTPUT     — validates InvestigationResult fields on sampled transactions
  LANGSMITH             — verifies traces appeared in the fraudlens project

Tests only the Investigation Agent (p 0.3–0.7, 5 tools, claude-haiku-4-5).
For the Critical Agent use: uv run scripts/critical_agent_healthcheck.py
...
def print_routing_section(
    ...
    for bucket, scenario, api_resp in results:
        ...
        line = f"{bucket:<22}  {'  '.join(parts)}{hint_tag}"
        if is_ok:
            passed += 1
            print(f"  {_ok(line)}")
        else:
...
def _fmt_tool_result(tool: str, result_str: str) -> list[str]:
    ...
    if tool == "explain_ml_score":
        ...
        if cases:
            top = cases[0]
            lines.append(f"{top.get('case_id')}  sim={top.get('similarity_score')}  fraud_confirmed={top.get('fraud_confirmed')}  modus={top.get('modus_operandi')}")
...
def _print_single_agent(bucket: str, scenario: dict[str, Any], api_resp: dict[str, Any]) -> None:
    ...
    print(f"\n  {_B}[{bucket}]{_X}  {_dim(f'score={prob:.4f}  is_fraud={is_fraud}')}")
...
def _header() -> None:
    line = "═" * 55
    print(f"\n{_B}{line}")
    print("  FraudLens — Investigation Agent Health Check")
    print("  Model: claude-haiku-4-5  |  Tools: 5  |  Tier: 0.3–0.7")
    print(f"{line}{_X}")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="FraudLens Investigation Agent smoke test")
    ...
    p.add_argument("--reliability-samples", type=int, default=0, help="Extra investigate txns for reliability check")
...
async def _async_main() -> int:
    ...
    routing_pairs: list[tuple[str, dict[str, Any], dict[str, Any]]] = [(bucket, row := rng.choice(scenarios[bucket]), make_payload(row)) for bucket in BUCKET_ORDER]

    invest_pool = sum((scenarios[b] for b in BUCKET_ORDER), [])
    extra_scenarios = rng.sample(invest_pool, min(args.reliability_samples, len(invest_pool)))
    reliability_pairs: list[tuple[str, dict[str, Any], dict[str, Any]]] = [(row["expected_bucket"], row, make_payload(row)) for row in extra_scenarios]
    ...
    async with httpx.AsyncClient() as client:
        for bucket, _scenario, p in routing_pairs:
            ...
            if "_error" in resp:
                err = resp["_error"][:40]
                print(f"    {_fail(f'{bucket:<20} API error: {err}')}")
            else:
        ...
        for i, (bucket, _scenario, p) in enumerate(reliability_pairs):
            ...
            if "_error" in resp:
                err = resp["_error"][:40]
                print(f"    {_fail(f'[Reliability #{i + 1}] {bucket:<15} API error: {err}')}")
            else:
                action = resp.get("triage_action", "?")
                prob = resp.get("fraud_probability", 0.0)
                hint = (resp.get("investigation") or {}).get("decision_hint", "none")
    ...
    print(f"\n{_B}[AGENT INVESTIGATION DETAIL]{_X}")
    for bucket, scenario, api_resp in routing_results:
        if "_error" not in api_resp:
            _print_single_agent(bucket, scenario, api_resp)

    rel_pass, rel_total = print_reliability_section(routing_results + reliability_results)
